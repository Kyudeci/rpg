        ...
        for rank in range(1, 6):
            ...
            if monstertype:
                if rank == 1:
                    ...
                    enemies = Monster(
                        image, monstertype[i], monstertype[i], rank, hp, atk, dfn, matk, mdef, spd, mxp)
    ...
    if defender.m_type == "Sphiren" or defender.m_type == "Sfiren":
        if defender.sphere_mode == False and defender.hp != 0:
            if counter in [2, 8, 14, 20]:
                messagebox.showinfo(
                    "Alert", defender.m_type + " is now in sphere mode!")
                defender.atk *= 0.5
                defender.matk *= 0.5
                defender.dfn *= 2
                defender.mdef *= 2
            ...
            if counter in [5, 11, 17, 23]:
                messagebox.showinfo(
                    "Alert", defender.m_type + " is no longer in sphere mode!")
                defender.atk *= 2
                defender.matk *= 2
                defender.dfn /= 2
                defender.mdef /= 2
